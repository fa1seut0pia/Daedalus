...
### 服务器
- 自定义服务器可**经 SOCKS5 代理连接**，代理地址和账号按服务器各自配置，例如 EasyTier 非 TUN 模式暴露的 `127.0.0.1:1080`，用来访问虚拟网络内的 DNS。代理只承载 TCP，这类服务器的 UDP 查询自动改走 TCP。服务器编辑页有"测试连接"按钮，按当前填写内容直接试一次查询。
